...
while Resident == "yes" :
  while True:
    Level = input("Level—freshman, sophomore, junior, senior: ").lower() #asking what level and is lowercased incase the user inputs uppercase letters 
  ...
  if Level == "freshman":
    ...
    if EGR_Admission != "yes" and JMC_Admission != "yes":
      ...
      if Credits_Int > 19:
        tuition = ((Credits_Int-18)*482) + 7230 + 5 + 24 #flat rate + tuition with the taxes of taking 6 credits or higher
      print("Tuition is ${:,.2f}.".format(tuition)) #this takes whatever number is printed and formats it so that coma is added in with the number
      PROMPT = input("Do you want to do another calculation (yes/no): ").lower() #after tuition is printed this will be asked and will stop if you say no or continue if you say "yes"
      if PROMPT != "yes": 
        break
      if PROMPT == "yes":
  ...
  if Level == "junior" or Level == "senior":
    ...
    if Level_Junior_Sciences == 'business' or Level_Junior_Sciences == 'engineering' or Level_Junior_Sciences == 'health' or Level_Junior_Sciences == 'sciences':
      Level_Junior_CMSE = input("Is your major CMSE (“Computational Mathematics and Engineering”) (yes/no): ").lower()
      while True:
        Credits = input("Credits: ")
        if not Credits.isdigit() or int(Credits) == 0:
          print("Invalid input. Try again.")
      ...
      if Level_Junior_Sciences == "engineering":
        ...
        if PROMPT == "yes":
            Resident = input("Resident (yes/no): ").lower()
            continue    
    if not (Level_Junior_Sciences == 'business' or Level_Junior_Sciences == 'engineering' or Level_Junior_Sciences == 'health' or Level_Junior_Sciences == "sciences" ):
      Level_Junior_CMSE = input("Is your major CMSE (“Computational Mathematics and Engineering”) (yes/no): ").lower()        
